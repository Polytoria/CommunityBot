...
const displayTexts: Record<string, string> = {
  'Invalid username.':
    "I don't see the player with that username, maybe try again.. or If you're searching using ID, Try type \" id\" after your targetted user id!",
  'Invalid user ID.':
    "I don't see the player with that id, maybe try again.. or If you're searching using username, Try type \" username\" after your targetted user s' username!"
}
...
export class responseHandler {
  ...
  public static checkError (response: AxiosResponse) {
    ...
    if (response.status >= 500) {
      ...
      result.displayText =
        'An unexpected error happen while sending request to Polytoria API. Please try again in a few minutes.'
